...
#include "Online/DebugActors/HexReplicatorDebugDormantActor.h"
...
void AHexSystemCharacter::HandleFire_Implementation()
{
    ...
    GetWorld()->SpawnActor<AHSProjectile>(spawnLocation, spawnRotation, spawnParameters);
}
...
void AHexSystemCharacter::ClientRPCEnableFlushing_Implementation(bool value)
{
    unimplemented();
}

void AHexSystemCharacter::ServerRPCFlushOnce_Implementation()
{
    for(TObjectIterator<AHexReplicatorDebugDormantActor> itr; itr; ++itr)
        (*itr)->FlushNetDormancyOnce();
}